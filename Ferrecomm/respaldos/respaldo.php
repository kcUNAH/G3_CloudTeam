<?php
	...
	$usuario="root"; //valor predeterminado
	$db = "ferrecomm_db";
	$passwd="";
	...
	$viewSQL  = "SHOW FULL TABLES FROM `".$db."` WHERE Table_Type='VIEW';"; //SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = :database AND TABLE_NAME = :table
	$tablaSQL = "SHOW FULL TABLES FROM $db WHERE Table_Type='BASE TABLE';"; //SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = :database AND TABLE_NAME = :table
	set_time_limit(300); //alarga el timeout

	// CONEXION 
	$conexion = new mysqli($host, $usuario, $passwd, $db);
	...
	$FicheroOUT=$db."_backup_".$info['fecha'].".sql"; //Este es el nombre del archivo a generar 
	$dump = <<<EOT
	# +===================================================================
	...
	// Falla si hay una function o un procedure que se llame igual que una vista.
		...
		$arraySQL=array("SHOW FUNCTION STATUS where Db like '". $db ."';","SHOW PROCEDURE STATUS where Db like '". $db ."';");
	...
	//VIEWs 
		...
		if ($ciclos>=30) {
			...
			$FicheroOUT="ERROR_bucle_$db.sql";
		}
		...
		$consulta = "SHOW FULL TRIGGERS FROM `".$db."`;"; //SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = :database AND TABLE_NAME = :table
		$respuesta = $conexion->query($consulta) or die ("No se puede ejecutar la consulta: $consulta MySQL:". $conexion->error); 
		while ($fila = $respuesta->fetch_array(MYSQLI_NUM)) {
			if ($drop) $dump .="\r\nDROP TRIGGER IF EXISTS `".$db."`.`".$fila[0]."`;\r\n";
			if ($DELIMITER!=";") 	$dump .="\r\nDELIMITER $DELIMITER\r\n";
			$dump .= "Create trigger `".$db."`.`".$fila[0]."`  ".$fila[4]." ".$fila[1]." ON `".$fila[2]."` \r\n";
			$dump .= "      for each row \r\n";
			$dump .= $fila[3];
			if ($DELIMITER!=";") 	$dump .="\r\n".$DELIMITER."\r\nDELIMITER  ";
			$dump .=";\r\n";
